...
# Suppress pyaudio warning messages
...
class SpeechRecognitionServer(Node):
    def __init__(self):
        ...
        self.get_logger().info('Starting speech recognition server.')
        # self.lang = 'ja'
        self.lang = 'en'
    ...
    def handle_accepted_callback(self, goal_handle):
        with self.goal_lock:
            if self.goal_handle is not None and self.goal_handle.is_active:
                self.get_logger().info('Stopping previous voice input')
    ...
    def execute_callback(self, goal_handle):
        with self.execute_lock:
            ...
            with Microphone() as source:
                self.get_logger().info('Voice input started, please speak.')
            ...
            try:
                ...
                text = self.recognizer.recognize_whisper(audio_data, model="medium", language=self.lang) #[*] The recorded audio data is sent to Whisper, which returns the recognition result.
                # text = self.recognizer.recognize_google(audio_data, language=self.lang)
            except RequestError:
                self.get_logger().info('API unavailable')
                return result

            except UnknownValueError:
                self.get_logger().info('Unrecognizable speech')
